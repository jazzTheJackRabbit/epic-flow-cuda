...
output = ""
threadXArray = [8,16,32,64]
threadYArray = [2,4,8,16,32]
if len(sys.argv) <=1 :
					...
					for i in range(0,len(files)):
						if(i < len(files) - 1):
							...
							if(".png" in  ref_image and ".png" in next_image):
								...
								if(sys.argv[1] == "-gpu"):
									print(["./run_all.sh", ref_image, next_image, str(base_frame_name + "-edge-file"), str(base_frame_name + "-match-file"), str(base_frame_name + "-flow.flo"),str(base_frame_name + ".png"),"-tx "+str(threadX),"-ty "+str(threadY),"-gpu"])
									subprocess.call(["./run_all.sh", ref_image, next_image, str(base_frame_name + "-edge-file"), str(base_frame_name + "-match-file"), str(base_frame_name + "-flow.flo"),str(base_frame_name + ".png"),"-tx "+str(threadX),"-ty "+str(threadY),"-gpu"])
								elif(sys.argv[1] == "-cpu"):
									subprocess.call(["./run_all.sh", ref_image, next_image, str(base_frame_name + "-edge-file"), str(base_frame_name + "-match-file"), str(base_frame_name + "-flow.flo"),str(base_frame_name + ".png"),"","",""])
