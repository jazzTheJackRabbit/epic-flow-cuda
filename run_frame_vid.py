...
if len(sys.argv) <=1 :
		...
		for i in range(0,len(files)):
			if(i < len(files) - 1):
				...
				if(".png" in  ref_image and ".png" in next_image):
					...
					if(sys.argv[1] == "-gpu"):
						subprocess.call(["./run_all.sh", ref_image, next_image, str(base_image_name + "-edge-file"), str(base_image_name + "-match-file"), str(base_image_name + "-flow.flo"),str(base_frame_name + ".png"),"-gpu"])
					elif(sys.argv[1] == "-cpu"):
	...
	if sys.argv[1] == "-gpu":
		subprocess.call(["cp","result/time.csv", "result/time-gpu.csv"]);
	elif sys.argv[1] == "-cpu":
